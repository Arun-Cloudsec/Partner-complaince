...
/* ───────────────────────── UI: Header ───────────────────────── */
const Header = ({ platform, onPlatformChange, onReset, hasData, onExport, onExportPdf, onExportWord, apiKey, onApiKeyChange, onBack }) => {
  ...
  <header className="border-b sticky top-0 z-30 backdrop-blur-sm" style={{ borderColor: T.line, background: `${T.bg}E6` }}>
    <div className="max-w-[1400px] mx-auto px-8 py-5 flex items-center justify-between">
      <div className="flex items-center gap-4">{onBack && <button onClick={onBack} className="px-3 py-2 text-xs font-mono" style={{ color: T.inkSoft, border: `1px solid ${T.line}` }}>← Back</button>}
...
export default function App({ onBack }) {
  ...
  return (
    <div className="font-body min-h-screen grain" style={{ background: T.bg, color: T.ink }}>
      <Header platform={platform} onPlatformChange={setPlatform} onReset={handleReset} hasData={view === "dashboard"} onExport={handleExport} onExportPdf={() => exportFormatted(requirements, verdict, metrics, "pdf")} onExportWord={() => exportFormatted(requirements, verdict, metrics, "word")} apiKey={apiKey} onApiKeyChange={setApiKey} onBack={onBack} />
